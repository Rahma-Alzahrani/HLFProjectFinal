...
# Close any previous networks and restart Docker
./network.sh down
systemctl restart docker

# Start org1 and org2 with the channel and CouchDB
./network.sh up createChannel -ca -c mychannel -s couchdb

# Deploy the chaincode in both org1 and org2 on the channel
./network.sh deployCC -ccn basic -ccp ../chaincode/chaincode-go -ccl go -c mychannel

# Change directory to addOrg3
cd addOrg3

# Add org3 to the existing channel with CouchDB
...
echo "Completed peer lifecycle chaincode package"
peer lifecycle chaincode package basic.tar.gz --path ../chaincode/chaincode-go/ --lang golang --label basic_1

# Install the chaincode and extract the package ID
echo "Completed peer lifecycle chaincode install"
