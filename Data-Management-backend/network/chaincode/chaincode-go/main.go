...
func (pm *dataManagement) JourneySchedule(ctx contractapi.TransactionContextInterface, journeySchedule string) (map[string]interface{}, error) {
	...
	if err != nil {
		response["message"] = "failed to get JourneySchedule"
		return response, fmt.Errorf("failed to get JourneySchedule: %v", err)
	}
	if exists {
		response["message"] = "JourneySchedule already exists"
		return response, fmt.Errorf("JourneySchedule already exists")
	}
	...
	if err != nil {
		...
		return response, fmt.Errorf("Failed to add JourneySchedule catch: %s", details.UID)
	}
...
func (pm *dataManagement) CreateHistoricalOfferRequest(ctx contractapi.TransactionContextInterface, historicalRequest string) (map[string]interface{}, error) {
	...
	// _historicalRequest.EndDate = _historicalRequest.HistoricalOfferDetails.EndDate
	// _historicalRequest.StartDate = _historicalRequest.HistoricalOfferDetails.StartDate

	// for _, offerID := range _historicalRequest.HistoricalOfferDetails.MultipleOfferID {
	// 	_historicalRequest.HistoricalOfferDetails.MultipleOfferID = append(_historicalRequest.HistoricalOfferDetails.MultipleOfferID, offerID)
	// }
	// Create a map to track unique offer IDs
	uniqueIDs := make(map[string]bool)

	// Create a new slice to store unique offer IDs
	uniqueOfferIDs := make([]string, 0)

	// Iterate through the original slice and add unique offer IDs to the map
	for _, offerID := range _historicalRequest.MultipleOfferID {
		if _, found := uniqueIDs[offerID]; !found {
			uniqueIDs[offerID] = true
			uniqueOfferIDs = append(uniqueOfferIDs, offerID)
		}
	}

	// Replace the original slice with the unique offer IDs
	_historicalRequest.HistoricalOfferDetails.MultipleOfferID = uniqueOfferIDs
...
// func (pm *dataManagement) InsertHistoricalDataHash2(ctx contractapi.TransactionContextInterface, offerID string, hashIDs, dataHashes, filenames, entrydates []string, offerDataHashIDs []string) error {
...
// 	for i, queryResponse := range resultsIterator {
// 		var hash OfferDataHash
// 		err = json.Unmarshal(queryResponse.Value, &hash)
// 		if err != nil {
// 			return err
...
// 		data := DataHash{
// 			DocType:   DATA_HASH_VALUE,
// 			ID:        offerDataHashIDs[i], // Use the provided offerDataHashID for each data hash
// 			Hash:      dataHashes[i],
// 			FileName:  filenames[i],
// 			EntryDate: entrydates[i],
// 		}
...
// 	for j, _agreement := range dataAgreement {
// 		logger.Info(fmt.Sprintf("_agreement: %v", _agreement))
// 		logger.Info(fmt.Sprintf("_agreement.OfferDataHashID: %v", _agreement.OfferDataHashID))
// 		_agreement.OfferDataHashID = append(_agreement.OfferDataHashID, offerDataHashIDs[j])
// 		logger.Info(fmt.Sprintf("_agreement.OfferDataHashID: %v", _agreement.OfferDataHashID))
// 		_aggrementAsBytes, err := json.Marshal(_agreement)
// 		logger.Info(fmt.Sprintf("_aggrementAsBytes: %v", _aggrementAsBytes))
// 		if err != nil {
...
func (pm *dataManagement) GetAllDataHashes(ctx contractapi.TransactionContextInterface) ([]*OfferDataHash, error) {
	...
	queryString := fmt.Sprintf(`{"selector":{"%s": "%s"}}`, DOC_TYPE, DATA_HASH)
